...
class SetupDemoCmsTwigService extends MelisCoreGeneralService
{
    ...
    public function setup($environmentName = 'development', $sitelabel = null)
    {
        ...
        if ($platform) {
            ...
            $outputFileName = 'MelisDemoCmsTwig.config.php';
            $configDir = $melisSite . '/MelisDemoCmsTwig/config/' . $outputFileName;
            $this->config = file_get_contents($configDir);

            // Getting the Pre-defined data for MelisDemoCmsTwigSite in config DIR
            $setupDatas = include __DIR__ . '../../../install/MelisDemoCmsTwig.setup.php';
    ...
    public function siteConfigCheck()
    {
        ...
        $moduleConfigDir = $melisSite . '/MelisDemoCmsTwig/config/' . $moduleConfigFileName;

        if (!is_writable($configDir) || !is_writable($moduleConfigDir)) {
            exit('Access permission denied, Please make /MelisDemoCmsTwig/config/' . $outputFileName . ' and /MelisDemoCmsTwig/config/' . $moduleConfigFileName . ' files writable');
        }
    }
    ...
    public function setupSite($site, $siteId = null)
    {
        ...
        if (is_null($siteId)) {
             ...
             * Becuase main page of MelisDemoCmsTwig is not yet created
             */
    ...
    private function setupPages($pages, $fatherId = -1)
    {
        ...
        $moduleConfigDir = $melisSite . '/MelisDemoCmsTwig/config/' . $outputFileName;
